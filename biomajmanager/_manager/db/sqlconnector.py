...
class SQLConnector:
    ...
    def _history(self, bank=None, to_json=False):
        ...
        for p, u in releases:
            rel_list.append({'id': "%d" % p.idproductionDirectory,
                             ...
                             'created': p.creation.strftime("%Y-%m-%d %H.%M.%S"),
                             'path': p.path,
                             'status': p.state})
        session.commit()
        session.close()
    ...
    def _mongo_history(self, bank=None):
        ...
        rel_list = []
        for p, u in releases:
            rel_list.append({'_id': '@'.join(['bank', bank.name, u.updateRelease, str(u.startTime).replace(' ','_')]),
                             'type': 'bank',
                             'name': bank.name,
                             ...
                             'bank_type': split(bank.config.get('db.type'), ','),
                             'bank_format': split(bank.config.get('db.formats'), ','),
                             'packages': bank._get_formats_for_release(p.path),
